	...
	frappe.call({
		...
		callback: function (response) {
			if (response.message && response.message.length > 0) {
				...
				frappe.show_alert({
					message: __(
						"No RIASEC Answer Options found in database. Please create some options first."
					),
					indicator: "orange",
				});
			}
		...
		error: function (err) {
			...
			frappe.show_alert({
				message: __(
					"Error fetching RIASEC Answer Options. Please check your connection and try again."
				),
				indicator: "red",
			});
		},
